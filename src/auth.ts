...
import { accounts, sessions, users, verificationTokens, clinicUsers, patients, clinicPatients, doctors, clinicDoctors } from "@/db/schema";
                    ...
                    if (passwordsMatch) {
                        ...
                        if (user.role === "patient") {
                            ...
                            if (user.role === "doctor") {
                                ...
                                // Médico pode estar vinculado à clínica apenas via clinic_doctors,
                                // sem entrada em clinic_users. Busca o clinicId dessa tabela.
                                if (!clinicId && doctorId) {
                                    const doctorClinicLink = await db.query.clinicDoctors.findFirst({
                                        where: eq(clinicDoctors.doctorId, doctorId),
                                    });
                                    clinicId = doctorClinicLink?.clinicId;
                                }
                            }
                        }
