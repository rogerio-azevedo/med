...
import { Plus, Loader2, Briefcase, Check } from "lucide-react";
import { createPracticeAreaAction } from "@/app/actions/practice-areas";
...
export function AddPracticeAreaDialog() {
    ...
    async function onSubmit(values: PracticeAreaFormValues) {
        setIsPending(true);
        try {
            const result = await createPracticeAreaAction(values);
            if (result.success) {
                toast.success("Área de atuação cadastrada com sucesso!");
                setIsOpen(false);
                form.reset();
            } else {
                toast.error(result.error || "Erro ao cadastrar área de atuação");
            }
        } catch (error) {
            toast.error("Erro ao cadastrar área de atuação");
        } finally {
            setIsPending(false);
        }
    }
            ...
            <DialogContent className="sm:max-w-md p-0 overflow-hidden border-none shadow-2xl bg-white/95 backdrop-blur-md">
                <div className="bg-gradient-to-r from-primary/10 via-primary/5 to-transparent p-6 pb-4 border-b">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <DialogTitle className="text-2xl font-bold tracking-tight text-foreground/90">Nova Área de Atuação</DialogTitle>
                            <DialogDescription className="text-muted-foreground/80 font-medium">
                                Cadastre uma nova área de atuação médica no sistema.
                            </DialogDescription>
                        </div>
                    </div>
                </div>
