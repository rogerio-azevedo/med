...
import { register, getInvite } from "@/app/actions/auth"
...
type CepResult = Awaited<ReturnType<typeof cep>>
...
export default function RegisterForm() {
  ...
  const lookupCep = async (cleanCEP: string) => {
    setLoadingCEP(true)
    try {
      const data: CepResult = await cep(cleanCEP)
      setFormData((prev) => ({
        ...prev,
        street: data.street || "",
  ...
  useEffect(() => {
    ...
    getSpecialtiesAction().then((r) => {
      if (r.success && r.data)
        setSpecialties(r.data.map((s) => ({ value: s.id, label: s.name })))
    })
  }, [inviteCode])
  ...
  const isStep2Valid =
    ...
    (inviteData?.role === "doctor"
      ? formData.crm && formData.crmState
          ...
          <div>
            ...
            <CardDescription className="flex flex-wrap items-center gap-1.5 font-medium">
              {inviteData?.clinicName ? (
                <>
                  A{" "}
                  <span className="text-primary font-bold underline underline-offset-2 decoration-primary/30">
                    {inviteData.clinicName}
                  </span>{" "}
                  te convidou.
                </>
        ...
        <form action={action} className="space-y-6">
            ...
            <div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300">
              <div className="grid gap-2">
                ...
                <p className="text-[11px] text-muted-foreground">
                  Este será seu login principal no sistema.
                </p>
              </div>
              <div className="grid gap-2">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div className="grid gap-2">
                      ...
                      <Select
                        ...
                        onValueChange={(v) => handleSelectChange("crmState", v)}
                      >
                  ...
                  <div className="grid gap-2">
                    ...
                    <ReactSelect
                      ...
                      value={specialties.filter((s) =>
                        formData.specialtyIds?.includes(s.value),
                      )}
                      onChange={(v) =>
          ...
          {step === 3 && (
            <div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300">
                  ...
                  <Input
                    ...
                    onBlur={(e) => {
                      if (e.target.value && formData.street && formData.city) {
                        geocodeAddress(
                          `${formData.street}, ${e.target.value}, ${formData.city}, Brasil`,
                        )
                      }
                    }}
                    required
              ...
              {mapCoords && (
                <input type="hidden" name="latitude" value={mapCoords.lat} />
              )}
              {mapCoords && (
                <input type="hidden" name="longitude" value={mapCoords.lng} />
              )}
            </div>
          )}
            ...
            <div className="flex gap-4 pt-2">
              {step > 1 && (
                <Button
                  ...
                  variant="outline"
                  onClick={prevStep}
                  className="flex-1 h-12 rounded-xl transition-all active:scale-95"
                  disabled={isPending}
                >
              ...
              {step < 3 ? (
                ...
                <Button
                  type="submit"
                  className="flex-1 h-12 rounded-xl bg-primary hover:bg-primary/90 transition-all active:scale-95 shadow-lg shadow-primary/20"
                  disabled={isPending}
                >
                  {isPending ? (
