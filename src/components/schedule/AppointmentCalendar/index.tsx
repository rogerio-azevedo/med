...
const HOUR_HEIGHT = 64; // px por hora (pouco maior para respirar)
...
export function AppointmentCalendar({
    ...
    function getSlotStyle(appt: AppointmentCardData): React.CSSProperties {
        ...
        const height = Math.max((appt.durationMinutes / 60) * HOUR_HEIGHT - 2, 20);
        return {
            position: "absolute",
            top: `${top}px`,
            left: "2px",
    ...
    const showNowLine = isCurrentWeek && now.getHours() >= START_HOUR && now.getHours() < END_HOUR;
                ...
                <div className="grid grid-cols-[60px_repeat(7,1fr)] min-w-[700px]">
                    ...
                    {showNowLine && (
                        ...
                        >
                            ...
                            <div className="absolute -left-[2px] -top-1 size-2 rounded-full bg-red-500"></div>
                        </div>
                    )}

                    {/* Coluna de horas */}
                    <div className="relative border-r border-border/50 bg-muted/10" style={{ height: TOTAL_HOURS * HOUR_HEIGHT }}>
                        {hours.map((h) => (
                            <div
                                key={h}
