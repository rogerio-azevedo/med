...
import { deleteMedicationAction } from "@/app/actions/medications";
...
export function DeleteMedicationDialog({ medication, isOpen, onOpenChange }: DeleteMedicationDialogProps) {
  ...
  async function onDelete() {
    ...
    try {
      ...
      if (result.success) {
        ...
        toast.error(result.error || "Erro ao remover medicamento");
      }
    } catch {
      ...
      <DialogContent className="overflow-hidden border-none bg-white p-0 shadow-2xl sm:max-w-[420px]">
        <div className="border-b border-destructive/20 bg-destructive/10 p-6 pb-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <DialogTitle className="text-xl font-bold tracking-tight text-destructive">Remover Medicamento</DialogTitle>
        ...
        <div className="p-6">
          <p className="text-sm leading-relaxed text-muted-foreground">
            Você tem certeza que deseja remover o medicamento <span className="font-bold text-foreground">&quot;{medication.name}&quot;</span>?
          </p>
        </div>
        ...
        <DialogFooter className="flex items-center justify-between border-t bg-muted/20 p-6 sm:justify-between">
          ...
          <Button variant="destructive" disabled={isPending} onClick={onDelete} className="min-w-[120px] text-white shadow-xl shadow-destructive/10 transition-all active:scale-95">
            {isPending ? (
              <>
